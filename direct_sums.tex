...
\]
where the triangle inequality again follows from $f_i g^*_i + g_i f^*_i \leq f_i f^*_i + g_i g^*_i$. We interpret the sum as before either in terms of a supremum over the norm of finite partial sums, or as converging in the double dual $\cat{C}(B,B)^{**}$.
...
\begin{rem}
	...
	We can thus define the \emph{direct sum with kernel $K$} to be an object representing this functor in the same way as above. For example in $\Hilb$ with $A_i = \C$ for all $i\in I$, the resulting direct sum would be the space of families $(\xi_i)_{i\in I}$ with $\xi_i\in\C$ that are $K$-square summable in that $\sum_{i,j} K_{ij} \bar{\xi}_i \xi_j < \infty$, turned into a Hilbert space with respect to the inner product with kernel $K$.
\end{rem}
