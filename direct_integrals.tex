...
$\NRep{\cat{N}}$ has a canonical measurable structure of this kind, which we describe in the following. First, a collection of representations $\mathcal{S}$ on a fixed Hilbert space $\H$ is measurable if it is in the smallest $\sigma$-algebra which makes the evaluation maps $\pi \mapsto \langle \xi,\pi(a) \xi\rangle$ for $\xi\in\H$ and $a\in\cat{N}$ measurable. Furthermore, we equip $\Obj{\Hilb}$ with the countable-cocountable $\sigma$-algebra. On $\Obj{\NRep{\cat{N}}}$, we hence put the $\sigma$-algebra consisting of countable or cocountable unions of measurable subsets of $\{ \pi : \cat{N} \to \B{\H} \}$ for varying $\H$.
