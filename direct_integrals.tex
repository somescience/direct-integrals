...
\begin{ex}
$\NRep{\cat{N}}$ has a canonical measurable structure of this kind if we declare a collection of objects $\mathcal{S}$ to be measurable if for every Hilbert space $\H$, the set of all $\pi : \cat{N} \to \B{\H}$ which are in is $\mathcal{S}$ is measurable, in the sense of the smallest $\sigma$-algebra which makes the evaluation maps $\pi \mapsto \langle \xi,\pi(a) \xi\rangle$ for $\xi\in\H$ and $a\in\cat{N}$ measurable. As a degenerate special case, for $\cat{C} = \Hilb$ we obtain the discrete $\sigma$-algebra on the objects.
