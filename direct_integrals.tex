...
We let $\LMeas$ be the category of localizable measurable spaces~\cite{nlab:localizable_measure}. So objects of $\LMeas$ are triples $(X,\Sigma,\Null)$, where $(X,\Sigma)$ is a $\sigma$-algebra and $\Null\subseteq\Sigma$ is a $\sigma$-ideal, whose elements are called \emph{null sets}, such that $\Sigma/\Null$ is complete as a Boolean algebra. Measurable sets with null complement are also called \emph{full sets}. We usually just write $X$ for a localizable measurable space, and indicate its $\sigma$-algebra and $\sigma$-ideal of measurable and null sets by $\Sigma_X$ and $\Null_X$, respectively. The morphisms of $\LMeas$ are the measurable maps $f : X \to Y$ modulo null sets, i.e.~$f,g : X\to Y$ represent the same morphism if $\{ x\in X \: | \: f(x) \neq g(x) \} \in \Null_X$.

A \emph{measure} $\mu$ on localizable measurable space $X$ is a measure $\mu : \Sigma_X \to \Rplusext$ in the usual sense, but such that every null set has zero measure. It hence descends to a map $\Sigma_X / \Null_X \to \Rplusext$ satisfying the inclusion-exclusion relation.

\tob{Is this map $\Sigma_X/\Null_X \to \Rplusext$ necessarily Scott continuous, so that it becomes a continuous valuation?}
